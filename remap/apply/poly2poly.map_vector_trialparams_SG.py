...
def getInputPolyIDs(nc_in):
    # make this varname an argument!!!
    inPolyIDs  = getNetCDFData(nc_in, 'CatchID')    
    print("read input inPolyIds ('CatchID') from parameter file")
    return inPolyIDs
...
def compAvgVal_scalar(wgt, var_in, varname):    
    ...
    for h in range(start_poly,(end_poly+1)):        
        ...
        for c in range(0, nCurrCells):
            ...
            print(dataVals[currInPolyIndex[c]])

            # check for NAN here 
            if dataVals[currInPolyIndex[c]] != dataVals[currInPolyIndex[c]]:
                print("WARNING: found NaN in input")
                currWgtVal[c] = 0
                numvoid   = numvoid + 1
...
Usage: %s -[h] <weight_netCDF> <input_netCDF> <output_netCDF>
	       ...
	       <start_poly (from 0)> <end_poly> <varname_file_csv>
        -h  help
'''
if __name__ == '__main__':
    ...
    if len(args) == 8:
        ...
        nc_outfl = args[2]
        hru_type = args[3]
        id_map_in = args[4]  # assigns the mappingfile_id to datafile index, starting with 0
        ...
        print("number of args is not 8: %d" % len(args))
        usage()
