...
#
#   AWW June 2020 read time details from input & pass to output...some other use of 
#                  'times' variable now is superfluous
#
# =========================================================================
...
class wgtnc:
    ...
    def getPolyWgtsIndices(self, currOutPolyID):
        ...
        Index = [i for i, x in enumerate(self.IDmask) if x == currOutPolyID]
        ...
        curr_intersector1 = [int(x-1) for x in curr_intersector] # fitting smhi formats
        ...
        # curr_input_index = list(dataIndex[curr_intersector])  OLD
...
def getNetCDFData(fn, varname):
    ...
    print("reading %s" % varname)
    
    f = nc4.Dataset(fn,'r')
    data = f.variables[varname][:]
    if(data.dtype != 'object'):
      if(np.isnan(data).any()):
          print("!! WARNING: found nans in data for variable %s, replacing with %f" % 
                (varname, voidVal))
      data[np.isnan(data)] = voidVal   # AWW to handle smhi data
...
#                    times, hrus, hru_type):    
#def writeNetCDFData(fn, var, times, hrus, hru_type, varname, time_ncvar):    
def writeNetCDFData(fn, var, hrus, hru_type, varname, time_ncvar):    
    """ Write <vars>[time,hru] array in netCDF4 file <fn> for variable <varname> """
    ...
    if hru_type == 'str':
        ...
        hruId = ncfile.createVariable('subid', 'S1', ('hru', 'strlen'))  
        hruId[:] = nc4.stringtochar(np.asarray(hrus,
                                  dtype='S{}'.format(max_strlen)))         
    else:
        # integer HRU
        hruId = ncfile.createVariable('subid', 'i8', ('hru', ))   # edited EC
        hruId[:] = np.asarray(hrus, dtype='int')
        #hruId[:] = np.asarray(hrus, dtype='i8')        # AW for HUCs
    
    hruId.long_name = 'spatial unit ID'
            
    # --- Times ---
    # t = ncfile.createVariable('time', 'i4', ('time', )) # int for seconds
    t = ncfile.createVariable('time', 'f4', ('time', ))   # float for 'days'
    t[:] = time_ncvar[:]  
    t.long_name = 'Date time of variable'
    t.units = time_ncvar.getncattr('units')
    t.calendar = time_ncvar.getncattr('calendar')
    
    # geophysical vars 
    vardata = ncfile.createVariable(varname,'f4',('time','hru',),fill_value='-9999.0')
    vardata[:,:] = var   # for mean area precip, etc.
    vardata.setMissing = '1.e+20'    
...
def getInputDataDims(nc_in):
    ...
    inPolyIDs  = getNetCDFData(nc_in, 'subid')    
    ...
    print("read time and inPolyIds ('subid') from forcing file")

    return timeVals,inPolyIDs  # note these are not the full variable, just the data
...
def compAvgVal(wgt, nc_in, varname):
    ...
    for h in range(start_poly,(end_poly+1)):        
        ...
        for c in range(0, nCurrCells):
            ...
            # handle different void situations for temperature versus other variables (bounded)
            # for now hardwire for SMHI data variable names, but generalize later
            
            if varname == 'timeCTMP':      
                if (currInPolyIndex[c] < 0 or dataVals[0,currInPolyIndex[c]] > 1.e+19 or 
                        dataVals[0,currInPolyIndex[c]] < -100 ):
                    currWgtVal[c] = 0
                    numvoid       = numvoid + 1
                    print(" -- outPoly %d %s: void found at input index %d" % 
                          (h, outPolyIDs[h], currInPolyIndex[c]))
            else:
                if (currInPolyIndex[c] < 0 or dataVals[0,currInPolyIndex[c]] > 1.e+19 or 
                        dataVals[0,currInPolyIndex[c]] < 0 ): 
                    currWgtVal[c] = 0
                    numvoid       = numvoid + 1
                    print(" -- outPoly %d %s: void found at input index %d" % 
                          (h, outPolyIDs[h], currInPolyIndex[c]))
        ...
        # create zero vector to start array for sums or void
        oriVals = 0 * dataVals[:,0].reshape(-1,1)   # yields time x 1d array
        ...
        if sumweights > 0:
            ...
            for c in range(0, nCurrCells):
                ...
                if currWgtVal[c]>0:
                    ...
                    # yields time x Nvalidcell array

            # now sum up weighted input polygon cells, delete temporary array
            Vals2=oriVals.sum(axis=1).reshape(-1,1)    # yields time x 1d array
            del oriVals

        # else none of the contributing cells are present and avg can't be formed
        else:
            
            Vals2 = oriVals*0+voidVal
            print(Vals2.shape)
            del oriVals
            
            print("WARNING: no valid input contributing areas found")
            print("WARNING: all cells (%d of %d)" % (numvoid, validcell) +
                  " in hru %d are void or missing: setting HRU values to %f in output" % (h,voidVal))

            nVoidOutPoly = nVoidOutPoly + 1
            #sys.exit()
        
        # now store time vector of weighted average or void results
...
if __name__ == '__main__':
    ...
    if len(args) == 8:
        ...
        voidVal      = -9999        # global, use in subroutines
        nVoidOutPoly = 0            # global, write out at end
        ...
        # get time variable from input to transfer to output
        f = nc4.Dataset(nc_in,'r')
        time_ncvar = f.variables['time']
        ...
        print("Averaging areas for %d polygons: %d to %d" % (nOutPolygons, start_poly, end_poly))

        # spatially average variables
        print("processing variable %s" % varname)
        hrus,var = compAvgVal(wgts, nc_in, varname)

        # write out netcdf file
        writeNetCDFData(nc_out, var, hrus, hru_type, varname, time_ncvar)   
        
        print("------------------\n  wrote %d void polygons out of %d in output " 
              % (nVoidOutPoly,end_poly-start_poly+1))
